	...
	for i := 1; i < len(words); i++ {
		for j := 0; j < len(words[i]); j++ {
			if !(words[i][j] >= 'A' && words[i][j] <= 'Z') && !(words[i][j] >= 'a' && words[i][j] <= 'z') &&
				...
				return "error", custome_errors.IncorrectSymbols()
			}
		}
	}
